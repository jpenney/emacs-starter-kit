...
(defcustom bcc-cache-directory (concat "~/.emacs.d/byte-cache/" system-name)
...
(defvar from-cedet-dir-sep-char (if (boundp 'directory-sep-char)
...
(defun from-cedet-directory-name-to-file-name (referencedir)
...
Converts directory seperation characters into ! characters."
  (let ((file referencedir)
    ...
    (when (memq system-type '(windows-nt ms-dos))
      ...
      (setq dir-sep-string (char-to-string from-cedet-dir-sep-char))
      ...
      (if (eq (aref file 1) ?:)
         (setq file (concat dir-sep-string
                            ...
                            (if (eq (aref file 2) from-cedet-dir-sep-char)
                                ""
                              dir-sep-string)
                            (substring file 2)))))
